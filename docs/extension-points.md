...
- `lens_node` (A3/A4 repo lenses) — `CoverageLens` (coverage report → under-covered changed files + score), `TechDebtLens` (jscpd + Sonar SARIF → duplication/issue findings + scorecard). Deterministic; each inert unless its report/definition is configured.
...
| **3 — check sources** ✅ | consume MegaLinter + Prowler-IaC (SARIF), coverage parsers, Trivy | **Done:** `utils/sources/sarif.py` (SARIF→Finding, source+rule preserved) + `utils/sources/coverage.py` (lcov/cobertura/jacoco); `@tool`s in `utils/tools.py` (`run_prowler_iac`→security ingest, `run_trivy`→security bundled-or-ingest, `run_megalinter`→style). Secret scanning (gitleaks) was dropped — it would feed credential values to the LLM. Coverage parser awaits the A3 lens (Phase 7). |
...
- **Releases are automated** (release-please, Conventional Commits): merging the "release" PR cuts the `vX.Y.Z` + `vX.Y` + `vX` git tags (major+minor floats moved by `tag-floats`) and pushes the matching `:vX.Y.Z`/`:vX.Y`/`:vX`/`:latest` image (the build is chained in `release-please.yml` because a GITHUB_TOKEN-created tag can't trigger `build-image.yml`). See [`RELEASING.md`](../RELEASING.md). Don't hand-tag.
